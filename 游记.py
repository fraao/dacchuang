...
headers = {
    'cookie': '_ga=GA1.2.50538359.1626942417; MKT_CKID=1626942416972.xsrkp.h14a; _RSG=vt4axMVXju2TUp4mgpTnUB; '
              '_RDG=28416d30204f5527dc27cd978da9f4f9ba; _RGUID=2e2d85f5-bb90-4df9-b7b1-773ab013379d; '
              'GUID=09031042315856507136; nfes_isSupportWebP=1; nfes_isSupportWebP=1; ibulanguage=CN; '
              'ibulocale=zh_cn; cookiePricesDisplayed=CNY; _gid=GA1.2.607075386.1635573932; MKT_Pagesource=PC; '
              'Session=smartlinkcode=U130026&smartlinklanguage=zh&SmartLinkKeyWord=&SmartLinkQuary=&SmartLinkHost=; '
              'Union=AllianceID=4897&SID=130026&OUID=&createtime=1635573932&Expires=1636178732460; '
              'MKT_CKID_LMT=1635573932634; _RF1=113.204.171.221; '
              'ASP.NET_SessionSvc=MTAuNjAuNDkuOTJ8OTA5MHxqaW5xaWFvfGRlZmF1bHR8MTYyMzE0MzgyNjI2MA; '
              '_bfa=1.1626942411832.2cm51p.1.1635573925821.1635580203950.4.26; _bfs=1.2; '
              '_jzqco=%7C%7C%7C%7C%7C1.429931237.1626942416968.1635580207564.1635580446965.1635580207564'
              '.1635580446965.0.0.0.19.19; __zpspc=9.4.1635580207.1635580446.2%232%7Cwww.baidu.com%7C%7C%7C%7C%23; '
              'appFloatCnt=7; _bfi=p1%3D290602%26p2%3D0%26v1%3D26%26v2%3D25',
...
def parse_one_page(html):
    ...
    for ii in ii_list:
        ...
        if '作者去了这些地方' in result:
            ...
            b = '未填写'
        print(title, day, time)
        # print(title, day, time, people, money,results)
        # info_list.append([title, day, time, people, money, results])
        # print(info_list)
        # return info_list
...
def save_to_csv(data):
    with open(r'C:\Users\ASUS\Desktop\呼和浩特游记数据.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['title', 'day', 'time', 'people', 'money', 'results'])
        for row in data:
            writer.writerow(row)
...
def main(offset):
    ...
    base_url = 'https://you.ctrip.com/travels/hohhot156/t2-p{}.html'
    print(offset)
    url = base_url.format(offset)
    html = etree.HTML(get_one_page(url))
    travel_data = parse_one_page(html)
    save_to_csv(travel_data)
    parse_one_page(html)
...
if __name__ == '__main__':
    ...
    # 此处优化，资源管理：使用with语句创建的进程池能自动释放资源，而不使用with语句创建的进程池则需要手动释放资源。
    with Pool(8) as p:
        p.map(main, [i for i in range(1, 5)])  # 爬取页数
    # 保存位置
    # 关闭线程池
    end_time = time.time()
    print('多线程爬取结束')
